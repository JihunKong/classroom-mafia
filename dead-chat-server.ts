...
export const handleDeadChatEvents = (io: Server, socket: Socket) => {
    ...
    try {
      ...
      if (room) {
        for (const socketId of room) {
          ...
          if (targetSocket && !targetSocket.data.player.isAlive) {
  ...
  socket.on('player:died', (playerId: string) => {
    ...
    const systemMessage: DeadChatMessage = {
      ...
      playerId: 'system',
      playerName: '시스템',
      message: `${socket.data.player.name}님이 사망하여 데드챗에 참여했습니다.`,
      timestamp: new Date(),
...
export const notifyPlayerDeath = (io: Server, roomCode: string, player: any) => {
  ...
  const playerSocket = [...io.sockets.sockets.values()]
    .find(s => s.data.player?.id === player.id);
  ...
  if (room) {
    const systemMessage: DeadChatMessage = {
      ...
      playerId: 'system',
      playerName: '시스템',
      message: `${player.name}(${player.role.name})님이 데드챗에 참여했습니다.`,
      timestamp: new Date(),
      role: { id: 'system', name: '시스템', team: 'neutral' }
    };
    ...
    for (const socketId of room) {
      const targetSocket = io.sockets.sockets.get(socketId);
      if (targetSocket && !targetSocket.data.player.isAlive && targetSocket.data.player.id !== player.id) {
        targetSocket.emit('deadChat:message', systemMessage);
      }
    }
  }
